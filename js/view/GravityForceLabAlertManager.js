...
class GravityForceLabAlertManager extends ISLCAlertManager {
  ...
  alertMassValueChanged( objectEnum, forceBiggerOverride ) {
    ...
    phet.joist.sim.utteranceQueue.addToBack( contextResponse );

    // voicing - create the alert to be used with voicing, including both the context response and object response.
    // It would be ideal if Voicing could be generated by the actual mass control, but we generate it in the
    // alert manager because this description needs to use information about whether the mass was "pushed", which
    // happens after the value updates in step.
    ...
    voicingUtteranceQueue.addToBack( voicingAlertString );
  }
